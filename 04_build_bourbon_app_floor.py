...
# MAGIC %md
# MAGIC # 04 — Build `app.bourbons` floor from the gold catalog
# MAGIC
# MAGIC Reads `prod_celr.gold.bourbon_catalog` and writes the pullable floor to
# MAGIC `prod_celr.app.bourbons` so that **odds live in the floor composition**.
# MAGIC The app pull stays uniform and trivial:
# MAGIC ```sql
# MAGIC select * from bourbons where tier = :t order by random() limit 1;
# MAGIC ```
...
# MAGIC which `00_celr_odds_config` already computed. One catalog bottle serves
# MAGIC several tiers (a $2,500 bottle is band6 at tier 1 but band3 at tier 4), so
# MAGIC each floor row is a `(bottle, tier)` placement and a bottle is replicated
# MAGIC within a cell to reach the target count.
# MAGIC
...
# ============================== CONFIG =======================================
SOURCE_TABLE = "prod_celr.gold.bourbon_catalog"
TARGET_TABLE = "prod_celr.app.bourbons"
...
# MAGIC %md ### 5. Shape to `app.bourbons` schema and write
...
# MAGIC %sql
# MAGIC select * from prod_celr.app.bourbons
